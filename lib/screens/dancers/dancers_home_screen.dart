...
class _DancersHomeScreenState extends State<DancersHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedCategory != 'All') {
      filteredDancers = filteredDancers.where((d) => d.danceStyles.contains(_selectedCategory)).toList();
    }
    ...
    if (_searchQuery.isNotEmpty) {
      filteredDancers = filteredDancers.where((d) {
        return d.name.toLowerCase().contains(_searchQuery.toLowerCase()) ||
            ...
            d.danceStyles.any((s) => s.toLowerCase().contains(_searchQuery.toLowerCase()));
      }).toList();
    }
          ...
          children: [
            Row(
              children: const [
                ...
                SizedBox(width: 4),
                Text('Chandigarh, Punjab', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                Icon(Icons.keyboard_arrow_down, size: 18),
              ],
            ),
            const Text('Discover Baraat & Wedding Dancers', style: TextStyle(fontSize: 11, color: AppColors.secondaryText)),
          ],
        ),
        actions: [
          IconButton(
            icon: const Icon(Icons.notifications_none_rounded, color: AppColors.primaryPlum),
            onPressed: () {
          ...
          child: Column(
            ...
            children: [
                          ...
                          ElevatedButton(
                            onPressed: () {
                              Navigator.of(context).push(
                                MaterialPageRoute(builder: (_) => const CreateDancerProfileFlow()),
                              );
                            },
                            ...
                            child: const Text('Create Dancer Profile', style: TextStyle(fontSize: 13)),
                          ),
                        ],
                      ),
                    ),
                    const SizedBox(width: 8),
                    const Icon(Icons.stars_rounded, color: AppColors.saffron, size: 64),
                  ],
                ),
              ),
              ...
              Row(
                ...
                children: [
                  const Expanded(
                    child: Text('Featured Dancers', style: AppTypography.h2, maxLines: 1, overflow: TextOverflow.ellipsis),
                  ),
                  TextButton(
                    onPressed: () {},
              ...
              if (filteredDancers.isEmpty)
                const Padding(
                  ...
                  child: Center(
                    child: Text('No dancers found matching filter criteria.'),
                  ),
                )
              else
                ListView.builder(
